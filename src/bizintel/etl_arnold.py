"""etl_arnold.py - example.

A script that loads prepared data into a data warehouse.
We call this ETL (Extract, Transform, Load).

Before this, call the dw_create_arnold.py script to create the empty data warehouse.
...
Data Source:
- data/prepared_2/customers_data_prepared.csv
- data/prepared_2/products_data_prepared.csv
- data/prepared_2/sales_data_prepared.csv
...
DATA_PREPARED: Final[Path] = Path("data/prepared_2")
...
STORES_PREPARED: Final[Path] = DATA_PREPARED / "stores_data_prepared.csv"
CAMPAIGNS_PREPARED: Final[Path] = DATA_PREPARED / "campaigns_data_prepared.csv"
...
def load_customers(
    ...
    df["customertenureyears"] = pd.to_numeric(
        df["customertenureyears"], errors="coerce"
    )
...
def load_products(
    ...
    df["averagequalityrating"] = pd.to_numeric(
        df["averagequalityrating"], errors="coerce"
    )
...
def load_sales(
    ...
    df["transactiontimeminutes"] = pd.to_numeric(
        df["transactiontimeminutes"], errors="coerce"
    )
...
def load_stores(
    ...
    """Load prepared stores data into dim_stores.

    Args:
        conn: Open DuckDB connection.
        df: Prepared stores DataFrame.

    Returns:
        None
    """
    LOG.info("Loading stores into dim_stores")
    ...
    LOG.info(f"  Loaded {df.shape[0]} rows into dim_stores")
...
def load_campaigns(
    ...
    """Load prepared campaigns data into dim_campaigns.

    Args:
        conn: Open DuckDB connection.
        df: Prepared campaigns DataFrame.

    Returns:
        None
    """
    LOG.info("Loading campaigns into dim_campaigns")
    ...
    LOG.info(f"  Loaded {df.shape[0]} rows into dim_campaigns")
...
def main() -> None:
    ...
    df_stores = load_data(STORES_PREPARED, "stores prepared")
    df_campaigns = load_data(CAMPAIGNS_PREPARED, "campaigns prepared")
    ...
    LOG.info(f"  Stores:    {df_stores.shape[0]} rows")
    LOG.info(f"  Campaigns: {df_campaigns.shape[0]} rows")
    ...
    LOG.info("CALL a function to load stores........")
    load_stores(conn, df_stores)

    LOG.info("CALL a function to load campaigns........")
    ...
    verify_row_count(conn, "dim_stores", df_stores.shape[0])
    verify_row_count(conn, "dim_campaigns", df_campaigns.shape[0])
